...
export default function Nutrition() {
  ...
  const { data, isFetching, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // const response = await api.get<ResponseDate>("/teste");
        const response = await api.post<ResponseDate>("/create", {
          name: user.name,
          age: user.age,
          gender: user.gender,
          height: user.height,
  ...
  if (isFetching) {
    return (
      <View style={styles.loading}>
        <Text style={styles.loadingText}>Gerando sua dieta com IA...</Text>
        <ActivityIndicator
        ...
        {data && Object.keys(data).length > 0 && (
          <Fragment>
            ...
            <ScrollView>
              ...
              >
                <Text style={styles.buttonText}>Gera nova dieta</Text>
              </Pressable>
            </ScrollView>
          </Fragment>
        )}
